...
import gasi.gps.dataupload.domain.port.inbound.BaseUplService;
...
public abstract class BaseUplServiceImpl
    ...
    private static final DateTimeFormatter INSTRUCTION_NO_TIMESTAMP_FORMAT =
    ...
    public DataUplDetailResponse upload(String resource, FileReadInput file) {
        ...
        DataUpl dataUpl = DataUpl.builder()
                ...
                .instructionNo(generateInstructionNo(normalizedResource))
    ...
    public DataUplTemplate downloadTemplate(String resource, Map<String, String> params) {
        DataUplProcessor processor = processorRegistry.get(normalize(resource));
        return processor.downloadTemplate(params != null ? params : Collections.emptyMap());
    }
    ...
    public void delete(String resource, String id) {
        ...
        if (!DISCARDABLE_STATUSES.contains(dataUpl.getUploadStatus())) {
            throw new BusinessException("Upload cannot be deleted in status: " + dataUpl.getUploadStatus());
        }
    ...
    private String generateInstructionNo(String resource) {
        String resourceCode = resource.toUpperCase().replaceAll("[^A-Z0-9]+", "-");
        String timestamp = LocalDateTime.now().format(INSTRUCTION_NO_TIMESTAMP_FORMAT);
        String suffix = UUID.randomUUID().toString().substring(0, 8).toUpperCase();
        return "UPL-" + resourceCode + "-" + timestamp + "-" + suffix;
    }
