...
import gasi.gps.dataupload.domain.model.DataUplInput;
...
public abstract class BaseUplController {
    ...
    /**
     * Downloads the template file for a resource.
     *
     * @param resource   resource code
    ...
    public ResponseEntity<byte[]> downloadTemplate(
        ...
        DataUplTemplate template = service.downloadTemplate(resource, Map.copyOf(parameters));
        MediaType mediaType = template.contentType() != null && !template.contentType().isBlank()
                ? MediaType.parseMediaType(template.contentType())
    ...
    /**
     * Deletes an upload attempt that has not been committed or entered a
