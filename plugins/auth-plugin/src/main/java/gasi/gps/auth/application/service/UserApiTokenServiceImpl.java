...
public class UserApiTokenServiceImpl extends
    ...
    public Optional<AuthenticatedPrincipal> authenticate(String rawToken) {
        ...
        return repositoryPort.findBy(tokenFilter)
                ...
                .map(token -> {
                    ...
                    UserApiToken saved = repositoryPort.save(token);
                    return new AuthenticatedPrincipal(
                            "local",
                            "",
                            saved.getUser().getUsername(),
                            saved.getUser().getRoles().stream().map(Role::getCode).collect(Collectors.toSet()),
                            Map.of());
                });
    }
    ...
    public UserApiTokenDetailResponse create(UserApiTokenCreateRequest request) {
        ...
        domain.setRevokedAt(null);
        domain.setLastUsedAt(null);
    ...
    public void delete(Long id) {
        UserApiToken token = repositoryPort.findById(id)
                .orElseThrow(() -> new BusinessException("Invalid API token id: " + idEncoder.encode(id)));
